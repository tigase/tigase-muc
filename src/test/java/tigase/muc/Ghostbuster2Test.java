...
public class Ghostbuster2Test {
	...
	public void testPingForOver1000() throws Exception {
		...
		Field f = Ghostbuster2.class.getDeclaredField("mucComponent");
		f.setAccessible(true);
		f.set(ghostbuster2, mucComponent);
		...
		setActiveRooms(ghostbuster2, activeRooms);
		...
		for (Ghostbuster2.MonitoredObject monitor : ghostbuster2.monitoredObjects.values()) {
			f = Ghostbuster2.MonitoredObject.class.getDeclaredField("lastActivity");
			f.setAccessible(true);
	...
	public void setActiveRooms(Ghostbuster2 ghostbuster2, final Map<BareJID, Room> activeRooms) throws Exception {
		IMucRepository mucRepository = new MockMucRepository() {
			@Override
			public Map<BareJID, Room> getActiveRooms() {
				return activeRooms;
			}
		};
		Field f = Ghostbuster2.class.getDeclaredField("repository");
		f.setAccessible(true);
