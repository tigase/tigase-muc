...
public class Ghostbuster {
	...
	private final Map<JIDDomain, Long> lastActivity = new ConcurrentHashMap<JIDDomain,
																											Long>();
			...
			public void timeOutExpired(Packet data) {
				try {
					if (log.isLoggable(Level.FINEST)) {
						log.finest("Received ping timeout for ping " +
											 data.getElement().getAttributeStaticStr("id"));
					}
	...
	private String checkError(final Packet packet) {
		...
		for (Element reason : errorElement.getChildren()) {
			if ((reason.getXMLNS() == null) ||
					!reason.getXMLNS().equals("urn:ietf:params:xml:ns:xmpp-stanzas")) {
				continue;
			}
	...
	protected void onPingReceived(final Packet response) throws TigaseStringprepException {
		final JIDDomain k = new JIDDomain(response.getStanzaFrom(),
																			response.getStanzaTo().getDomain());
		...
		if (lastActivity.containsKey(k)) {
			...
			if (errorCause != null) {
				if (log.isLoggable(Level.FINEST)) {
					log.finest("Received error response for ping " +
										 response.getElement().getAttributeStaticStr(Packet.ID_ATT) + "(" +
	...
	private void ping(String fromDomain, JID jid) throws TigaseStringprepException {
		...
		Element ping = new Element("iq", new String[] { "type", "id", "from", "to" },
															 new String[] { "get",
						id, fromDomain, jid.toString() });

		ping.addChild(new Element("ping", new String[] { "xmlns" },
	...
	public void update(Packet packet) throws TigaseStringprepException {
		...
		final JIDDomain k = new JIDDomain(packet.getStanzaFrom(),
		...
		if (checkError(packet) != null) {
			...
			processError(k);
		} else if ("presence".equals(packet.getElemName()) && (type != null) &&
							 type.equals("unavailable")) {
			if (log.isLoggable(Level.FINEST)) {
	...
	private static class JIDDomain {
		...
		public JIDDomain(JID source, String domain) {
			this.source = source;
			this.domain = domain;
			cacheKey    = ((this.source == null)
										 ? "-"
										 : this.source.toString()) + ":" + ((this.domain == null)
							? "-"
