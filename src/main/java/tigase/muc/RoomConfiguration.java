...
public class RoomConfiguration implements Serializable {
	...
	public void flushConfig() {
		...
		try {
			...
			this.mucRepocitory.setData(namespace, id, "roomconfigChangeSubject", Boolean.toString(roomconfigChangeSubject));
			this.mucRepocitory.setData(namespace, id, "roomconfigAllowInvites", Boolean.toString(roomconfigAllowInvites));
			...
			this.mucRepocitory.setData(namespace, id, "roomconfigEnableLogging", Boolean.toString(roomconfigEnableLogging));
			this.mucRepocitory.setData(namespace, id, "roomconfigModeratedRoom", Boolean.toString(roomconfigModeratedRoom));
			this.mucRepocitory.setData(namespace, id, "roomconfigPersistentRoom", Boolean.toString(roomconfigPersistentRoom));
			this.mucRepocitory.setData(namespace, id, "roomconfigPasswordProtectedRoom",
					Boolean.toString(roomconfigPasswordProtectedRoom));
	...
	public Form getFormElement() {
		...
		x.addField(Field.fieldListSingle("muc#roomconfig_maxusers", String.valueOf(this.roomconfigMaxUsers),
				"Maximum Number of Room Occupants", new String[] { "1", "10", "20", "30", "50", "100", "150" }, new String[] { "1",
						"10", "20", "30", "50", "100", "150" }));
		...
		x.addField(Field.fieldListSingle("muc#roomconfig_whois", this.roomconfigWhois,
				"Affiliations that May Discover Real JIDs of Occupants", new String[] { "Room Owner and Admins Only",
						"Room Owner, Admins and Members Only", "Anyone" }, new String[] { "admin", "member", "anyone" }));
		...
		if (isRoomconfigPersistentRoom()) {
			try {
				if (affiliation == null || affiliation == Affiliation.NONE) {
					...
					this.mucRepocitory.setData(namespace, id + "/affiliation", jid.getBareJID().toString(), affiliation.name());
				}
			} catch (UserNotFoundException e) {
				e.printStackTrace();
			} catch (TigaseDBException e) {
